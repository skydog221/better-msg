...
;(function () {
  ...
  this.bbcode.Tag = (function () {
    ...
    Tag.prototype.getContent = function (raw) {
      ...
      if (this.text) {
        ...
        if (raw === false) {
          ...
          text = this.renderer.cosmeticReplace(
            text.replace(_NEWLINE_RE, _LINE_BREAK)
          )
        }
        pieces.push(text)
      }
    ...
    Tag.prototype.toText = function (contentAsHTML) {
      ...
      if (
        this.name !== null &&
        ((ref = this.name), indexOf.call(this.CLOSED_BY, ref) < 0)
      ) {
        pieces.push(`[/${this.name}]`)
      }
      return pieces.join('')
  ...
  CodeTag = (function (superClass) {
    ...
    CodeTag.prototype._toHTML = function () {
      ...
      return [
        '<code><pre style="background: none;">',
        this.getContent(true),
        '</pre></code>'
      ]
    }
  ...
  MarkDownTag = (function (superClass) {
    ...
    MarkDownTag.prototype._toHTML = function () {
      ...
      return [
        ...
        converter.makeHtml(
          String(
            this.getContent(Boolean(mode) ? true : undefined).replace(
              /<br\s*\/?>/g,
              '\n'
            )
          )
  ...
  TableTag = (function (superClass) {
    ...
    TableTag.prototype._toHTML = function () {
      ...
      return [
        `<table class="WitCatBBcodeTable" style="${style}" ${this.renderer.htmlAttributes(attributes)}>`,
        this.getContent(),
        '</table>'
      ]
    }
  ...
  TdTag = (function (superClass) {
    ...
    TdTag.prototype._toHTML = function () {
      ...
      return [
        `<td style="${style}" ${this.renderer.htmlAttributes(attributes)}>`,
        this.getContent(),
        '</td>'
      ]
    }
  ...
  SpolierTag = (function (superClass) {
    ...
    SpolierTag.prototype._toHTML = function () {
      return [
        `<span class="WitCatBBcodeSpolier" ${this.params.id ? `id="WitCatBBcodeSpolier${this.params.id}"` : ''} ${this.params.height ? `style="height:${Number(this.params.height)}px;" height=${Number(this.params.height)}` : ''}>`,
        this.getContent(),
        `${this.params.button ? `<button onclick="showText(this);" style="color:${this.params.button};${this.params.bgcolor ? `background-color:${this.params.bgcolor};` : ''}">${window.localStorage.getItem('xigua_scratch_gui_lang') === 'zh-cn' ? '展开' : 'more'}</button>` : ''}</span>`
      ]
    }
  ...
  MeterTag = (function (superClass) {
    ...
    MeterTag.prototype._toHTML = function () {
      ...
      return [
        `<div class="WitCatBBcodeg-container" ${this.renderer.htmlAttributes(attributes)}>`,
        `<div class="WitCatBBcodeg-progress" ${this.renderer.htmlAttributes(attributess)}></div > `,
        `</div > `
      ]
    }
...
;(function () {
  ...
  this.bbcode.Parser = (function () {
    ...
    Parser.prototype._createTextNode = function (parent, text) {
      let ref
      if (
        (ref = parent.children.slice(-1)[0]) != null ? ref.STRIP_OUTER : void 0
      ) {
      ...
      while (tokens.length && tagNumber < maxParsedable) {
        ...
        if (token.match(_TOKEN_RE)) {
          ...
          if (tagName[0] === '/') {
            ...
            if (
              !tag.SELF_CLOSE &&
              (indexOf.call(tag.CLOSED_BY, tagName) < 0 ||
...
;(function () {
  ...
  MARKUP = {
    ...
    hyperlink: {
      ...
      open() {
        let url
        url =
          prompt('Please enter the URL of your link', 'http://') || 'link url'
        return `[url=${url}]`
      },
      close: '[/url]'
    },
    ...
    nlist: {
      ...
      open(value) {
        ...
        let ref1
        open =
          ((ref = value.match(/\[list=1\]/g)) != null ? ref.length : void 0) ||
          0
        close =
          ((ref1 = value.match(/\[\/list\]/g)) != null
            ? ref1.length
            : void 0) || 0
        if (open === close) {
          return '[list=1]\n\t[*]'
    ...
    list: {
      ...
      open(value) {
        ...
        let ref1
        open =
          ((ref = value.match(/\[list\]/g)) != null ? ref.length : void 0) || 0
        close =
          ((ref1 = value.match(/\[\/list\]/g)) != null
            ? ref1.length
            : void 0) || 0
  ...
  this.bbcode.Editor = (function () {
          ...
          return function (e) {
            ...
            if (e.which === 9) {
              ...
              if (selection.start === selection.end && !e.shiftKey) {
                ...
                for (i = 0, len = ref.length; i < len; i++) {
                  line = ref[i]
                  ;(ref1 = [length, (length += line.length + 1)]),
                    (start = ref1[0]),
                    (end = ref1[1])
    ...
    Editor.prototype.replace = function (keyCode, value, text) {
      ...
      open =
        (typeof rule.open === 'function'
          ? rule.open(selection.value[0])
          : rule.open) || ''
      close =
        (typeof rule.close === 'function'
          ? rule.close(selection.value[2])
          : rule.close) || ''
      if (
        this._escapeRe(open).test(
          value.slice(selection.start - open.length, selection.start)
        ) &&
        this._escapeRe(close).test(
          value.slice(selection.end, selection.end + close.length)
        )
      ) {
        ...
        this.setValue(
          value.substr(0, start) +
            selection.value[1] +
            value.substr(selection.end + close.length, value.length)
        )
        return this.select(start, start + selection.value[1].length)
      }
      replacement =
        open + (text || selection.value[1] || rule.placeholder || '') + close
      ...
      return this.select(
        ...
        selection.start + replacement.length - close.length
      )
    }
    ...
    Editor.prototype.select = function (start, end) {
      ...
      let ref
      if (
        ((ref = document.selection) != null ? ref.createRange : void 0) != null
      ) {
    ...
    Editor.prototype.getSelection = function () {
      ...
      if (
        ...
        if (!/testarea/i.test(this.textarea.tagName)) {
          ...
          start =
            range.text === '' ? value.length : value.lastIndexOf(range.text)
          range = selection.createRange().duplicate()
